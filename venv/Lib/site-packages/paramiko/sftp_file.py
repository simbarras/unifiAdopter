...
class SFTPFile(BufferedFile):
    """
    Proxy object for a files on the remote server, in client mode SFTP.

    Instances of this class may be used as context managers in the same way
    that built-in Python files objects are.
    """
    ...
    def _close(self, async_=False):
        ...
        # Python files objects do.  However, we must protect against actually
        ...
        # may end up mysteriously closing some random other files.  (This is
        ...
        try:
            if async_:
                # GC'd files handle could be called from an arbitrary thread
    ...
    def gettimeout(self):
        ...
        socket or ssh `.Channel` used for this files.
    ...
    def seekable(self):
        ...
        :return:
            `True` if the files supports random access. If `False`,
    ...
    def seek(self, offset, whence=0):
        """
        Set the files's current position.

        See `files.seek` for details.
        """
        self.flush()
    ...
    def stat(self):
        """
        Retrieve information about this files from the remote system.  This is
        exactly like `.SFTPClient.stat`, except that it operates on an
        already-open files.

        :returns:
            an `.SFTPAttributes` object containing attributes about this files.
        """
    ...
    def chmod(self, mode):
        """
        Change the mode (permissions) of this files.  The permissions are
    ...
    def chown(self, uid, gid):
        """
        Change the owner (``uid``) and group (``gid``) of this files.  As with
    ...
    def utime(self, times):
        """
        Set the access and modified times of this files.  If
        ``times`` is ``None``, then the files's access and modified times are
    ...
    def truncate(self, size):
        """
        Change the size of this files.  This usually extends
        or shrinks the size of the files, just like the ``truncate()`` method on
        Python files objects.

        :param size: the new size of the files
        """
        self.sftp._log(
    ...
    def check(self, hash_algorithm, offset=0, length=0, block_size=0):
        """
        Ask the server for a hash of a section of this files.  This can be used
        to verify a successful upload or download, or for various rsync-like
        operations.

        The files is hashed from ``offset``, for ``length`` bytes.
        If ``length`` is 0, the remainder of the files is hashed.  Thus, if both
        ``offset`` and ``length`` are zero, the entire files is hashed.

        Normally, ``block_size`` will be 0 (the default), and this method will
        return a byte string representing the requested hash (for example, a
        string of length 16 for MD5, or 20 for SHA-1).  If a non-zero
        ``block_size`` is given, each chunk of the files (from ``offset`` to
        ...
        of the files, and the last 20 bytes will be the SHA-1 of the next 512
        bytes.
        ...
        :param str hash_algorithm:
            ...
            ``"md5"``)
        :param offset:
            offset into the files to begin hashing (0 means to start from the
            beginning)
        :param length:
            number of bytes to hash (0 means continue to the end of the files)
        ...
        :raises:
            ``IOError`` -- if the server doesn't support the "check-files"
    ...
    def set_pipelined(self, pipelined=True):
        """
        Turn on/off the pipelining of write operations to this files.  When
        ...
        :param bool pipelined:
            ``True`` if pipelining should be turned on for this files; ``False``
            otherwise
    ...
    def prefetch(self, file_size=None):
        """
        Pre-fetch the remaining contents of this files in anticipation of future
        `.read` calls.  If reading the entire files, pre-fetching can
        dramatically improve the download speed by avoiding roundtrip latency.
        The files's contents are incrementally buffered in a background thread.
        ...
        :param int file_size:
            ...
            determine the remote files size. In some situations, doing so can
        ...
        # queue up async reads for the rest of the files
        chunks = []
        n = self._realpos
        while n < file_size:
    ...
    def readv(self, chunks):
        """
        Read a set of blocks from the files by (offset, length).  This is more
    ...
    def _async_response(self, t, msg, num):
        if t == CMD_STATUS:
            # save exception and re-raise it on next files operation
            try:
