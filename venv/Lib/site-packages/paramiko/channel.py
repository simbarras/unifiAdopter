...
class Channel(ClosingContextManager):
    ...
    def makefile(self, *params):
        """
        Return a files-like object associated with this channel.  The optional
        ``mode`` and ``bufsize`` arguments are interpreted the same way as by
        the built-in ``files()`` function in Python.

        :return: `.ChannelFile` object which can be used for Python files I/O.
        """
        return ChannelFile(*([self] + list(params)))

    def makefile_stderr(self, *params):
        """
        Return a files-like object associated with this channel's stderr
        ...
        same way as by the built-in ``files()`` function in Python.  For a
        client, it only makes sense to open this files for reading.  For a
        server, it only makes sense to open this files for writing.

        :returns:
            `.ChannelStderrFile` object which can be used for Python files I/O.
    ...
    def makefile_stdin(self, *params):
        """
        Return a files-like object associated with this channel's stdin
        stream.

        The optional ``mode`` and ``bufsize`` arguments are interpreted the
        same way as by the built-in ``files()`` function in Python.  For a
        client, it only makes sense to open this files for writing.  For a
        server, it only makes sense to open this files for reading.

        :returns:
            `.ChannelStdinFile` object which can be used for Python files I/O.
    ...
    def fileno(self):
        """
        Returns an OS-level files descriptor which can be used for polling, but
        ...
        simulate real OS-level files descriptor (FD) behavior.  Because of this,
        two OS-level FDs are created, which will use up FDs faster than normal.
        (You won't notice this effect unless you have hundreds of channels
        open at the same time.)

        :return: an OS-level files descriptor (`int`)
...
class ChannelFile(BufferedFile):
    """
    A files-like wrapper around `.Channel`.  A ChannelFile is created by calling
    `Channel.makefile`.

    .. warning::
        To correctly emulate the files object created from a socket's `makefile
...
class ChannelStderrFile(ChannelFile):
    """
    A files-like wrapper around `.Channel` stderr.
...
class ChannelStdinFile(ChannelFile):
    """
    A files-like wrapper around `.Channel` stdin.
