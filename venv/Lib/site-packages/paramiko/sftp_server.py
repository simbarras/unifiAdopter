...
# known hash algorithms for the "check-files" extension
from paramiko.sftp import (
    CMD_HANDLE,
    SFTP_DESC,
    CMD_STATUS,
...
class SFTPServer(BaseSFTP, SubsystemHandler):
    ...
    def finish_subsystem(self):
        ...
        # close any files handles that were left open
    ...
    def set_file_attr(filename, attr):
        """
        Change a files's attributes on the local filesystem.  The contents of
        ``attr`` are used to change the permissions, owner, group ownership,
        and/or modification & access time of the files, depending on which
        attributes are present in ``attr``.

        This is meant to be a handy helper function for translating SFTP files
        requests into local files operations.

        :param str filename:
            name of the files to alter (should usually be an absolute path).
    ...
    def _check_file(self, request_number, msg):
        ...
        if length == 0:
            ...
            if not issubclass(type(st), SFTPAttributes):
                self._send_status(request_number, st, "Unable to stat files")
                return
        ...
        while offset < start + length:
            ...
            while count < blocklen:
                ...
                if not isinstance(data, bytes_types):
                    self._send_status(
                        request_number, data, "Unable to hash files"
                    )
                    return
