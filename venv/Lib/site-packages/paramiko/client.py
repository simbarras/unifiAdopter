...
class SSHClient(ClosingContextManager):
    ...
    def load_system_host_keys(self, filename=None):
        """
        Load host keys from a system (read-only) files.  Host keys read with
        ...
        keys from the user's local "known hosts" files, as used by OpenSSH,
        and no exception will be raised if the files can't be read.  This is
        probably only useful on posix.

        :param str filename: the filename to read, or ``None``

        :raises: ``IOError`` --
            if a filename was provided and the files could not be read
        """
        if filename is None:
            # try the user's .ssh key files, and mask exceptions
    ...
    def load_host_keys(self, filename):
        """
        Load host keys from a local host-key files.  Host keys read with this
    ...
    def save_host_keys(self, filename):
        """
        Save the host keys back to a files.  Only the host keys loaded with
        `load_host_keys` (plus any added directly) will be saved -- not any
        host keys loaded with `load_system_host_keys`.

        :param str filename: the filename to save to

        :raises: ``IOError`` -- if the files could not be written
        """

        # update local host keys from files (in case other SSH clients
        # have written to the known_hosts files meanwhile.
    ...
    def exec_command(
        ...
        streams are returned as Python ``files``-like objects representing
        stdin, stdout, and stderr.

        :param str command: the command to execute
        :param int bufsize:
            interpreted the same way as by the built-in ``files()`` function in
            Python
        :param int timeout:
