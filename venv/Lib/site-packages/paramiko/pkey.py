...
class PKey(object):
    ...
    def from_private_key_file(cls, filename, password=None):
        """
        Create a key object by reading a private key files.  If the private
        ...
        :param str filename: name of the files to read
        :param str password:
            an optional password to use to decrypt the key files, if it's
            encrypted
        :return: a new `.PKey` based on the given private key

        :raises: ``IOError`` -- if there was an error reading the files
        :raises: `.PasswordRequiredException` -- if the private key files is
            encrypted, and ``password`` is ``None``
        :raises: `.SSHException` -- if the key files is invalid
        """
    ...
    def from_private_key(cls, file_obj, password=None):
        """
        Create a key object by reading a private key from a files (or files-like)
        object.  If the private key is encrypted and ``password`` is not
        ``None``, the given password will be used to decrypt the key (otherwise
        `.PasswordRequiredException` is thrown).

        :param file_obj: the files-like object to read from
        :param str password:
        ...
        :raises: `.PasswordRequiredException` --
            if the private key files is encrypted, and ``password`` is ``None``
        :raises: `.SSHException` -- if the key files is invalid
        """
    ...
    def write_private_key_file(self, filename, password=None):
        """
        Write private key contents into a files.  If the password is not
        ``None``, the key is encrypted before writing.

        :param str filename: name of the files to write
        :param str password:
            an optional password to use to encrypt the key files

        :raises: ``IOError`` -- if there was an error writing the files
    ...
    def write_private_key(self, file_obj, password=None):
        """
        Write private key contents into a files (or files-like) object.  If the
        password is not ``None``, the key is encrypted before writing.

        :param file_obj: the files-like object to write into
        :param str password: an optional password to use to encrypt the key

        :raises: ``IOError`` -- if there was an error writing to the files
        :raises: `.SSHException` -- if the key is invalid
        """
        raise Exception("Not implemented in PKey")
    ...
    def _read_private_key_file(self, tag, filename, password=None):
        """
        Read an SSH2-format private key files, looking for a string of the type
    ...
    def _read_private_key(self, tag, f, password=None):
        ...
        if start >= len(lines) or keytype is None:
            raise SSHException("not a valid {} private key files".format(tag))
    ...
    def _read_private_key_pem(self, lines, end, password):
        ...
        while start < len(lines):
            ...
            start += 1
        # if we trudged to the end of the files, just try to cope.
        try:
        ...
        try:
            ...
            raise SSHException("Can't parse DEK-info in private key files")
        ...
        if password is None:
            raise PasswordRequiredException("Private key files is encrypted")
    ...
    def _read_private_key_openssh(self, lines, password):
        ...
        if kdfname == b("bcrypt"):
            if cipher == b("aes256-cbc"):
                ...
                raise SSHException(
                    "unknown cipher `{}` used in private key files".format(
                        cipher.decode("utf-8")
                    )
                )
            ...
            if password is None:
                raise PasswordRequiredException(
                    "private key files is encrypted"
                )
            ...
            raise SSHException(
                "unknown cipher or kdf used in private key files"
            )
        ...
        if checkint1 != checkint2:
            raise SSHException(
                "OpenSSH private key files checkints do not match"
            )
    ...
    def _write_private_key_file(self, filename, key, format, password=None):
        """
        Write an SSH2-format private key files in a form that can be read by
        ...
        :param filename: name of the files to write.
        :param str data: data blob that makes up the private key.
        :param str password: an optional password to use to encrypt the files.
    ...
    def load_certificate(self, value):
        ...
        public key (``.pub``) or certificate (``-cert.pub``) files, a string
        containing such a files, or a `.Message` object.

        The .pub contents adds no real value, since the private key
        files includes sufficient information to derive the public
...
# readable from a one-line files of the format:
...
class PublicBlob(object):
    ...
    def __init__(self, type_, blob, comment=None):
        ...
        :param str comment: A comment, if one was given (e.g. files-based.)
        """
        self.key_type = type_
        self.key_blob = blob
    ...
    def from_file(cls, filename):
        """
        Create a public blob from a ``-cert.pub``-style files on disk.
        """
