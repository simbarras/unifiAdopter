...
class BufferedFile(ClosingContextManager):
    """
    Reusable base class to implement Python-style files buffering around a
    simpler stream.
    """
    ...
    def __init__(self):
        ...
        self._closed = False
        # pos - position within the files, according to the user
    ...
    def __iter__(self):
        ...
        files.  This iterator happens to return the files itself, since a files is
        its own iterator.

        :raises: ``ValueError`` -- if the files is closed.
        """
        if self._closed:
    ...
    def close(self):
        """
        Close the files.  Future read and write operations will fail.
        """
        self.flush()
        self._closed = True
    ...
    if PY2:
        ...
        def next(self):
            ...
            ``StopIteration`` when EOF is hit.  Unlike Python files
            objects, it's okay to mix calls to `next` and `readline`.

            :raises: ``StopIteration`` -- when the end of the files is reached.

            :returns: a line (`str`) read from the files.
            """
            line = self.readline()
            if not line:
                raise StopIteration
        ...
        def __next__(self):
            ...
            when EOF is hit.  Unlike python files objects, it's okay to mix
    ...
    def readable(self):
        """
        Check if the files can be read from.

        :returns:
            `True` if the files can be read from. If `False`, `read` will raise
            an exception.
        """
    ...
    def writable(self):
        """
        Check if the files can be written to.

        :returns:
            `True` if the files can be written to. If `False`, `write` will
            raise an exception.
        """
    ...
    def seekable(self):
        """
        Check if the files supports random access.

        :returns:
            `True` if the files supports random access. If `False`, `seek` will
            raise an exception.
        """
        return False
    ...
    def read(self, size=None):
        """
        Read at most ``size`` bytes from the files (less if we hit the end of
        the files first).  If the ``size`` argument is negative or omitted,
        read all the remaining data in the files.

        .. note::
            ``'b'`` mode flag is ignored (``self.FLAG_BINARY`` in
            ``self._flags``), because SSH treats all files as binary, since we
            have no idea what encoding the files is in, or even if the files is
            text data.

        :param int size: maximum number of bytes to read
        :returns:
            data read from the files (as bytes), or an empty string if EOF was
    ...
    def readline(self, size=None):
        """
        Read one entire line from the files.  A trailing newline character is
        kept in the string (but may be absent when a files ends with an
        ...
        :returns:
            next line of the files, or an empty string if the end of the
            files has been reached.

            If the files was opened in binary (``'b'``) mode: bytes are returned
            Else: the encoding of the files is assumed to be UTF-8 and character
    ...
    def readlines(self, sizehint=None):
        ...
        :returns: list of lines read from the files.
        """
        lines = []
        byte_count = 0
        while True:
    ...
    def seek(self, offset, whence=0):
        """
        Set the files's current position, like stdio's ``fseek``.  Not all files
        objects support seeking.

        .. note::
            If a files is opened in append mode (``'a'`` or ``'a+'``), any seek
            operations will be undone at the next write (as the files position
            will move back to the end of the files).

        :param int offset:
            position to move to within the files, relative to ``whence``.
        :param int whence:
            type of movement: 0 = absolute; 1 = relative to the current
            position; 2 = relative to the end of the files.

        :raises: ``IOError`` -- if the files doesn't support random access.
        """
        raise IOError("File does not support seeking.")

    def tell(self):
        """
        Return the files's current position.  This may not be accurate or
        useful if the underlying files doesn't support random access, or was
        opened in append mode.

        :returns: files position (`number <int>` of bytes).
        """
        return self._pos

    def write(self, data):
        """
        Write data to the files.  If write buffering is on (``bufsize`` was
    ...
    def writelines(self, sequence):
        """
        Write a sequence of strings to the files.  The sequence can be any
    ...
    def xreadlines(self):
        """
        Identical to ``iter(f)``.  This is a deprecated files interface that
    ...
    def _get_size(self):
        """
        (subclass override)
        Return the size of the files.  This is called from within `_set_mode`
        if the files is opened in append mode, so the files position can be
        tracked and `seek` and `tell` will work correctly.  If the files is
    ...
    def _set_mode(self, mode="r", bufsize=-1):
        ...
        if bufsize == 1:
            ...
            # over a files will indirectly call readline).
        ...
        if "U" in mode:
            ...
            # built-in files objects have this attribute to store which kinds of
