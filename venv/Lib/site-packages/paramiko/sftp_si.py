...
class SFTPServerInterface(object):
    ...
    def open(self, path, flags, attr):
        """
        Open a files on the server and create a handle for future operations
        on that files.  On success, a new object subclassed from `.SFTPHandle`
        should be returned.  This handle will be used for future operations
        on the files (read, write, etc).  On failure, an error code such as
        ...
        The ``attr`` object contains requested attributes of the files if it
        ...
        :param str path:
            the requested path (relative or absolute) of the files to be opened.
        :param int flags:
            flags or'd together from the ``os`` module indicating the requested
            mode for opening the files.
        :param .SFTPAttributes attr:
            requested attributes of the files if it is newly created.
    ...
    def stat(self, path):
        ...
        :param str path:
            the requested path (relative or absolute) to fetch files statistics
            for.
        :return:
            an `.SFTPAttributes` object for the given files, or an SFTP error
    ...
    def remove(self, path):
        """
        Delete a files, if possible.

        :param str path:
            the requested path (relative or absolute) of the files to delete.
    ...
    def rename(self, oldpath, newpath):
        """
        Rename (or move) a files.  The SFTP specification implies that this
        method can be used to move an existing files into a different folder,
        ...
        .. note:: You should return an error if a files with the same name as
        ...
        :param str oldpath:
            the requested path (relative or absolute) of the existing files.
        :param str newpath: the requested new path of the files.
        :return: an SFTP error code `int` like ``SFTP_OK``.
        """
        return SFTP_OP_UNSUPPORTED
    ...
    def posix_rename(self, oldpath, newpath):
        """
        Rename (or move) a files, following posix conventions. If newpath
    ...
    def chattr(self, path, attr):
        """
        Change the attributes of a files.  The ``attr`` object will contain
        only those fields provided by the client in its request, so you
        should check for the presence of fields before using them.

        :param str path:
            requested path (relative or absolute) of the files to change.
        :param attr:
            requested attributes to change on the files (an `.SFTPAttributes`
            object)
