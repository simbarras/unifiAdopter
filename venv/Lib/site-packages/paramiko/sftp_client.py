...
#
# This files is part of Paramiko.
#
...
class SFTPClient(BaseSFTP, ClosingContextManager):
    ...
    def _log(self, level, msg, *args):
        if isinstance(msg, list):
            ...
            # escape '%' in msg (they could come from files or directory names)
            # before logging
            msg = msg.replace("%", "%%")
    ...
    def listdir_attr(self, path="."):
        ...
        field: ``longname``, which may contain a formatted string of the files's
    ...
    def listdir_iter(self, path=".", read_aheads=50):
        ...
        should suffice for most files listings as each request/response cycle
        ...
        while True:
            try:
                ...
                # responses later on Section 6.7 of the SSH files transfer RFC
                # explains this
    ...
    def open(self, filename, mode="r", bufsize=-1):
        """
        Open a files on the remote server.  The arguments are the same as for
        Python's built-in `python:files` (aka `python:open`).  A files-like
        object is returned, which closely mimics the behavior of a normal
        Python files object, including the ability to be used as a context
        manager.

        The mode indicates how the files is to be opened: ``'r'`` for reading,
        ``'w'`` for writing (truncating an existing files), ``'a'`` for
        appending, ``'r+'`` for reading/writing, ``'w+'`` for reading/writing
        (truncating an existing files), ``'a+'`` for reading/appending.  The
        ...
        succeed if the files was created and did not previously exist.  This has
        no direct mapping to Python's files flags, but is commonly known as the
        ``O_EXCL`` flag in posix.

        The files will be buffered in standard Python style by default, but
        can be altered with the ``bufsize`` parameter.  ``0`` turns off
        buffering, ``1`` uses line buffering, and any number greater than 1
        (``>1``) uses that specific buffer size.

        :param str filename: name of the files to open
        :param str mode: mode (Python-style) to open in
        :param int bufsize: desired buffering (-1 = default buffer size)
        :return: an `.SFTPFile` object representing the open files

        :raises: ``IOError`` -- if the files could not be opened.
        """
    ...
    # Python continues to vacillate about "open" vs "files"...
    file = open

    def remove(self, path):
        """
        Remove the files at the given path.  This only works on files; for
        removing folders (directories), use `rmdir`.

        :param str path: path (absolute or relative) of the files to remove
    ...
    def rename(self, oldpath, newpath):
        """
        Rename a files or folder from ``oldpath`` to ``newpath``.
        ...
        :param str oldpath:
            existing name of the files or folder
        :param str newpath:
            new name for the files or folder, must not exist already
    ...
    def posix_rename(self, oldpath, newpath):
        """
        Rename a files or folder from ``oldpath`` to ``newpath``, following
        posix conventions.

        :param str oldpath: existing name of the files or folder
        :param str newpath: new name for the files or folder, will be
    ...
    def stat(self, path):
        """
        Retrieve information about a files on the remote system.  The return
    ...
    def lstat(self, path):
        """
        Retrieve information about a files on the remote system, without
    ...
    def symlink(self, source, dest):
        ...
        :param str source: path of the original files
    ...
    def chmod(self, path, mode):
        """
        Change the mode (permissions) of a files.  The permissions are
        unix-style and identical to those used by Python's `os.chmod`
        function.

        :param str path: path of the files to change the permissions of
    ...
    def chown(self, path, uid, gid):
        """
        Change the owner (``uid``) and group (``gid``) of a files.  As with
        Python's `os.chown` function, you must pass both arguments, so if you
        only want to change one, use `stat` first to retrieve the current
        owner and group.

        :param str path: path of the files to change the owner and group of
    ...
    def utime(self, path, times):
        """
        Set the access and modified times of the files specified by ``path``.
        If ``times`` is ``None``, then the files's access and modified times
        ...
        :param str path: path of the files to modify
        :param tuple times:
    ...
    def truncate(self, path, size):
        """
        Change the size of the files specified by ``path``.  This usually
        extends or shrinks the size of the files, just like the `~files.truncate`
        method on Python files objects.

        :param str path: path of the files to modify
        :param int size: the new size of the files
        """
        path = self._adjust_cwd(path)
    ...
    def readlink(self, path):
        ...
        :param str path: path of the symbolic link files
    ...
    def putfo(self, fl, remotepath, file_size=0, callback=None, confirm=True):
        """
        Copy the contents of an open files object (``fl``) to the SFTP server as
        ``remotepath``. Any exception raised by operations will be passed
        through.

        The SFTP operations use pipelining for speed.

        :param fl: opened files or files-like object to copy
        ...
        :param callable callback:
            ...
            (since 1.7.4)
        :param bool confirm:
            whether to do a stat() on the files afterwards to confirm the files
            size (since 1.7.7)
    ...
    def put(self, localpath, remotepath, callback=None, confirm=True):
        """
        Copy a local files (``localpath``) to the SFTP server as ``remotepath``.
        Any exception raised by operations will be passed through.  This
        method is primarily provided as a convenience.

        The SFTP operations use pipelining for speed.

        :param str localpath: the local files to copy
    ...
    def getfo(self, remotepath, fl, callback=None):
        """
        Copy a remote files (``remotepath``) from the SFTP server and write to
        an open files or files-like object, ``fl``.  Any exception raised by
        operations will be passed through.  This method is primarily provided
        as a convenience.

        :param object remotepath: opened files or files-like object to copy to
        :param str fl:
            the destination path on the local host or open files object
        :param callable callback:
            optional callback function (form: ``func(int, int)``) that accepts
            the bytes transferred so far and the total bytes to be transferred
        :return: the `number <int>` of bytes written to the opened files object
    ...
    def get(self, remotepath, localpath, callback=None):
        """
        Copy a remote files (``remotepath``) from the SFTP server to the local
        host as ``localpath``.  Any exception raised by operations will be
        passed through.  This method is primarily provided as a convenience.

        :param str remotepath: the remote files to copy
        ...
        while True:
            ...
            try:
                if num not in self._expecting:
                    # might be response for a files that was closed before
                    # responses came back
