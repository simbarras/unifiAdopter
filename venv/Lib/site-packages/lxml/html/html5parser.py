...
def parse(filename_url_or_file, guess_charset=None, parser=None):
    ...
    when parsing from URLs, off by default when parsing from files(-like)
    objects (which tend to return Unicode more often than not), and on by
    default when parsing from a files path (which is read in binary mode).
    """
    if parser is None:
        parser = html_parser
    if not isinstance(filename_url_or_file, _strings):
        ...
        if guess_charset is None:
            # assume that files-like objects return Unicode more often than bytes
            guess_charset = False
