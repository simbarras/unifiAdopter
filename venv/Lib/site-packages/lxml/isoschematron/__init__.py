...
class Schematron(_etree._Validator):
    ...
    Alternatively, pass a filename as keyword argument 'files' to parse from
    the files system.
    ...
    def __init__(self, etree=None, file=None, include=True, expand=True,
        ...
        try:
            ...
            raise _etree.SchematronParseError(
                "No tree or files given: %s" % sys.exc_info()[1])
        if root is None:
    ...
    def __call__(self, etree):
        ...
        if errors:
            if _etree.iselement(etree):
                fname = etree.getroottree().docinfo.URL or '<files>'
            else:
                fname = etree.docinfo.URL or '<files>'
            for error in errors:
